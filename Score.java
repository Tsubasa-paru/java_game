...
/*public Position makexy(){
        Position xy = new Position();
        for(int i=0;flag_hit = true;i++){
           x = (int)(Math.random()*1500)+150;//150~1650までのx座標整数乱数
           y = (int)(Math.random()*715)+150;//150~865までのy座標整数乱数
            flag_hit =
                 (400-plane_size<x && x<450+plane_size && 350-plane_size<y && y<400+plane_size)
              || (134-plane_size<x && x<184+plane_size && 517-plane_size<y && y<567+plane_size)
              || (134-plane_size<x && x<184+plane_size && 128-plane_size<y && y<178+plane_size)
              || (310-plane_size<x && x<360+plane_size && 730-plane_size<y && y<780+plane_size)
              || (570-plane_size<x && x<620+plane_size && 10-plane_size<y && y<60+plane_size)
              || (595-plane_size<x && x<645+plane_size && 750-plane_size<y && y<800+plane_size)
              || (650-plane_size<x && x<700+plane_size && 200-plane_size<y && y<250+plane_size)
              || (690-plane_size<x && x<740+plane_size && 380-plane_size<y && y<430+plane_size)
              || (770-plane_size<x && x<820+plane_size && 800-plane_size<y && y<850+plane_size)
              || (830-plane_size<x && x<880+plane_size && 80-plane_size<y && y<130+plane_size)
              || (500-plane_size<x && x<550+plane_size && 550-plane_size<y && y<600+plane_size)
              || (890-plane_size<x && x<940+plane_size && 560-plane_size<y && y<610+plane_size)
              || (950-plane_size<x && x<1000+plane_size && 10-plane_size<y && y<60+plane_size)
              || (1000-plane_size<x && x<1050+plane_size && 170-plane_size<y && y<220+plane_size)
              || (1050-plane_size<x && x<1100+plane_size && 300-plane_size<y && y<350+plane_size)
              || (1100-plane_size<x && x<1150+plane_size && 480-plane_size<y && y<530+plane_size)
              || (1150-plane_size<x && x<1200+plane_size && 720-plane_size<y && y<770+plane_size)
              || (1200-plane_size<x && x<1250+plane_size && 370-plane_size<y && y<420+plane_size)
              || (1250-plane_size<x && x<1300+plane_size && 850-plane_size<y && y<900+plane_size)
              || (1300-plane_size<x && x<1350+plane_size && 560-plane_size<y && y<610+plane_size)
              || (1350-plane_size<x && x<1400+plane_size && 50-plane_size<y && y<100+plane_size)
              || (1400-plane_size<x && x<1450+plane_size && 250-plane_size<y && y<300+plane_size)
              || (1450-plane_size<x && x<1500+plane_size && 820-plane_size<y && y<870+plane_size)
              || (1500-plane_size<x && x<1550+plane_size && 450-plane_size<y && y<500+plane_size)
              || (1550-plane_size<x && x<1600+plane_size && 660-plane_size<y && y<710+plane_size)
              || (1600-plane_size<x && x<1650+plane_size && 180-plane_size<y && y<230+plane_size)
              || (100-plane_size<x && x<150+plane_size && 780-plane_size<y && y<830+plane_size);
             if (flag_hit == false){
                 break;
             }
        }
        xy.ix = x;
        xy.iy = y;
        return xy;//クラスを返す、返されたクラスには定数ix,iyがランダムに保存
     }*/
...
Airplane() {
        /*Position xy = new Position();
        Position value = xy.makexy();
        ix = value.ix;                                   //初期座標と初期速度を設定
        iy = value.iy;*/
...
void update(int width,int height){  //PlanePanelのactionPerformed内で0.1秒ごとに呼び出し。座標を更新。
        //if(Math.random()<1) {}
        x += vx; y += vy; //カクカク動くようにランダムで更新
        if(x<0 || x+size>width) {vx *= -1;}           //画面端で跳ね返る
...
{595,800,50},{650,250,50},{690,430,50},{770,850,50},{830,130,50},{500,600,50},{890,610,50},{950,60,50},{1000,220,50},{1050,350,50},{1100,530,50},{1150,770,50},{1200,420,50},{1250,900,50},{1300,610,50},{1350,100,50},{1400,300,50},{1450,870,50},{1500,500,50},{1550,710,50},{1600,230,50},{100,830,50}};
   return obstacle;
  }
...
int getIX(){
        ...
        ix = p.ix;
        /*Position p = new Position();
        Position pv = p.makexy();
        ix = pv.ix;*/                                   //初期座標と初期速度を設定
  return ix;
}
int getIY(){
        int s = (int)(Math.random()*2);
        ...
        Position rv = r.makexy();                                   //初期座標と初期速度を設定
        iy = rv.iy;*/
  return iy;
}
...
public void paintComponent(Graphics g) {
        ...
        //g.drawImage(bufferedImage,400,400,null);
...
class Ia extends Component {
    BufferedImage bufferedImage = null;
    Ia() {//JPEG読み込みその画像を持つbufferedImageオブジェクト作成
        try {
            bufferedImage = ImageIO.read(new File("Ia.png"));//ImageIO.readはbufferedImage型を
        } catch (IOException e) {                                 //返すメソッド
            System.out.println("image file not found. [" + "Ia.png" + "]");
        }
    }
    public void paint(Graphics graphics) {
